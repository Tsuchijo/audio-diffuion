...
def main():
    ...
    # Avg together Left and Right channels to get Mono Signal, normalize
    audio_data = np.mean(audio_data, axis=1)/np.max(np.abs(audio_data))
    ...
    cumulative_loss = ddpm.train(mel_scale.to('cuda'), 10000)
    ## Save the model
